...
import * as t from '../actions/app.actions.gen';
...
it('applies a UpdateQueryAction against the store state', () => {
  const state = reducer(undefined, t.updateQuery(t.mockId(), t.mockId(), rand.str(), rand.str()));
  t.is<t.ClientState>(state);
});
...
it('applies a ExecuteSuccessQueryAction against the store state', () => {
  const state = reducer(undefined, t.executeSuccessQuery(t.mockId(), rand.str()));
  t.is<t.ClientState>(state);
});
